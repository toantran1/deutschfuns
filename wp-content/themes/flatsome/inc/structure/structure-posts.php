...
if ( ! function_exists( 'flatsome_posted_on' ) ) :
	...
	// 		wp_kses_post( _x( '<span class="posted-on">Đã Đăng vào %1$s</span> <span class="byline">bởi %2$s</span>', 'post date by post author', 'flatsome' ) ),
    ...
    function flatsome_posted_on() {
        ...
        $date_object = DateTime::createFromFormat('Y-m-d H:i:s', $published_date);
        
        // Định dạng ngày tháng theo yêu cầu
        $formatted_date = $date_object->format('j') . ' tháng ' . $date_object->format('n') . ' ' . $date_object->format('Y');
    
        $time_string = sprintf( $time_string,
            esc_attr( get_the_date( 'c' ) ),
            esc_html( $formatted_date ),
            esc_attr( get_the_modified_date( 'c' ) ),
            esc_html( $formatted_date ) // Có thể thay đổi nếu bạn muốn hiển thị ngày sửa đổi khác
        );
